...
func (h *TreeHandler) GetTree(ctx context.Context, req *connect.Request[appv1.GetTreeRequest]) (*connect.Response[appv1.GetTreeResponse], error) {
	...
	for _, item := range items { tree.Items = append(tree.Items, toProtoItem(item)) }
...
func (h *TreeHandler) GetSubtree(ctx context.Context, req *connect.Request[appv1.GetSubtreeRequest]) (*connect.Response[appv1.GetSubtreeResponse], error) {
	...
	if wsID == "" { return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("workspace_id is required")) }
	if req.Msg.GetItemId() == "" { return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("item_id is required")) }
	userID, err := requireUserIDOrShareToken(ctx)
	if err != nil { return nil, err }
	...
	for i, item := range items { protoItems[i] = toProtoSubtreeItem(item) }
...
func (h *TreeHandler) FindPaths(ctx context.Context, req *connect.Request[appv1.FindPathsRequest]) (*connect.Response[appv1.FindPathsResponse], error) {
	if req.Msg.GetSourceItemId() == "" || req.Msg.GetTargetItemId() == "" { return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("source_item_id and target_item_id are required")) }
	if req.Msg.GetWorkspaceId() == "" { return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("workspace_id is required")) }
	userID, err := requireUserIDOrShareToken(ctx)
	if err != nil { return nil, err }
	...
	for _, item := range items { protoTree.Items = append(protoTree.Items, toProtoItem(item)) }
