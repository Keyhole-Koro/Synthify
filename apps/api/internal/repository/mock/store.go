...
// WithTx は mock 環境では実 tx を持たないため、callback をそのまま *Store で
// 実行する。callback が error を返した場合の rollback は行えないため、
// state を巻き戻したいテストでは Store を作り直すこと。
// (本物の挙動は postgres.Store.WithTx を参照)
...
var _ repository.DocumentChunkRepository = (*Store)(nil)
var _ repository.JobRepository = (*Store)(nil)
var _ repository.JobApprovalRepository = (*Store)(nil)
var _ repository.JobLogRepository = (*Store)(nil)
