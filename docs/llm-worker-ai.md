...
LLM worker は、文書をそのまま全文一発処理するのではなく、段階的に graph 候補へ変換する。
...
- chunk 群から document-level graph を直接合成する
...
- node / edge は必ず `source_chunk_ids` を持つ
...
- `SynthesizedNode.SourceChunkIDs`
...
公開 API では node 本体に provenance を埋め込まず、evidence 系で返す方向が正しい。
