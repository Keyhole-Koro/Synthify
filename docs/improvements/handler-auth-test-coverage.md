...
- ✅ `ListAllJobs` に admin 認可を実装 (`middleware.IsAdmin` + `SYNTHIFY_ADMIN_USER_EMAILS` allowlist)。log-viewer は Postgres 直接参照に移行したため anonymous バイパスは廃止。
...
- ✅ ~~log endpoint の anonymous-read context での挙動テスト~~ → log-viewer 直接 DB 参照化により anonymous バイパス自体を廃止 ([log-viewer-direct-db.md](log-viewer-direct-db.md))。
