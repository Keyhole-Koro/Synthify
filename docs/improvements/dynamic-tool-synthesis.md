...
生成コード
  │
  ├─(a) 静的解析: コードを走査して機械的下限 floor を算出
  │      tier_1: 危険シグナル無し
  │      tier_2: I/O・パース系シグナル
...
これらは [プロンプト外出し (llm-eval-runner.md 前提作業 1)](llm-eval-runner.md) のテンプレートに**規約として埋め込む**。失敗予算 (上記) と合わせ、「修正再生成は 1 回・連続失敗で封印」を機械側でも強制する (プロンプトの善意に頼らない)。
