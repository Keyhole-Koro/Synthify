...
- [frontend-reload-performance.md](frontend-reload-performance.md) — フロント側リロード時の画面表示速度改善。workspace/paper/tree snapshot の cache-first 表示、verified 後 refresh queue、Firestore listener 遅延、bundle 分割、reload milestone 計測の実装計画
- [tree-node-workspace-ownership.md](tree-node-workspace-ownership.md) — **Done**: node を document 従属から外し workspace 直属の 1 本の統合ツリーに。0018 で `document_root`/`workspace_root`/`kind`/`document_tree_links` を廃止し `cross_document` を追加。worker/api/web も node 直属モデルに転換済み(bc0d8b3〜08d4ea0)。残: `cross_document` の UI 可視化、`item_aliases` による alias 統合は未実装。（旧 `workspace-root-id-unification` / `tree-item-kind-renaming` 案は本転換に吸収・不要化され削除済み）
...
- [stif-tree-item-import-format.md](stif-tree-item-import-format.md) — **Phase 1 実装済み**: STIF(Synthify Tree Item Format)。Codex / Claude / Gemini などの情報を tree item import 用 `.stif` に変換する形式とプロンプト。Go 側 parser + sanitizer(`apps/api/internal/stif`)、`TreeImportService.ImportStif`(dry_run で preview)、workspace paper の import UI(`apps/web/src/features/stif`)。残: `merge`/`replace` mode、`order`/`scope` の DB 保存、GitHub source の interactive retrieval
