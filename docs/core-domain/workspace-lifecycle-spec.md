...
- **論理削除の採用**: 物理削除ではなく、`deleted_at` カラムによる論理削除を採用。課金証跡の維持と非同期ジョブの安全な終了を優先。
- **バックエンド (実装済み)**: `DeleteWorkspace` RPC、サービス、リポジトリ層、および `deleted_at` マイグレーションが完了。
- **フロントエンド UI (将来のタスク)**: 一旦削除導線を除去。将来的に確認ダイアログと共に追加予定。
