...
> カバレッジ数値は `go test -coverprofile`（または各言語のカバレッジツール）の実測値を `(YYYY-MM-DD)` 付きで記す。
...
対象の interface / public API の **全メソッド** を列挙し、専用テストの有無と実測カバレッジを突き合わせる。
**テスト0件のメソッドは下のテストケース表に1行も現れない**ので、抜けはここで検出する。
（テストケース表を埋める前にこの表を先に作ると、未テストメソッドを取りこぼさない。）

| メソッド | 専用テスト | coverage | 状態 | 未テストの主分岐 |
| --- | --- | --- | --- | --- |
| `<Method>` | `<✅ N件 / ⚠ setup流用のみ / ❌ なし>` | `<実測%>` | `<OK / PARTIAL / GAP>` | `<認可拒否 / repo error / nil 依存 などの未踏分岐>` |

補助関数 / 内部経路で実測が低いものも一行で残す: `<helper>` `<%>` (`<残っている分岐>`).

## 依存エラー軸 (dependency returns error)

各メソッドが外部依存 (repo / provider / client) のエラーをどう伝播するかを横断で見る。
mock にフォールト注入が無い場合はここがまとめて ❌ になりやすい (failing decorator か mock 拡張が前提)。
☑=テスト有 / ◐=間接 / ❌=未テスト。

| メソッド | `<dep A>` err | `<dep B>` err | `<dep C>` err |
| --- | --- | --- | --- |
| `<Method>` | `<☑/◐/❌ + 呼び出し名>` | `<…>` | `<…>` |

## 未テスト分岐 (GAP) — テストケース表に現れない経路

メソッド単位ではなく分岐単位の穴。カバレッジ HTML の赤行に対応させる（任意。穴が分岐レベルで散らばる場合に追加）。

| 場所 (`file:line`) | 分岐 | 期待挙動 | なぜ重要か |
| --- | --- | --- | --- |
| `<file:line>` | `<条件>` | `<期待>` | `<リスク>` |
