...
-- name: ExpireUploadReservations :execrows
...
-- name: LockUploadReservation :one
...
-- name: MarkUploadReservationFailed :exec
UPDATE upload_reservations
SET status = 'failed', actual_size_bytes = $2
WHERE document_id = $1;

-- name: ConfirmUploadReservation :exec
UPDATE upload_reservations
...
-- name: IncrementAccountStorageUsed :exec
UPDATE accounts
SET storage_used_bytes = storage_used_bytes + $2,
    updated_at = $3
WHERE account_id = $1;

-- name: SumActiveReservedBytes :one
